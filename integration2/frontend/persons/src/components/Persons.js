...
const Persons=()=>{
    ...
    useEffect(()=>{
        if(query!==""){
            axios.get(`http://127.0.0.1:8000/pro/search/?Course=${query}`).then(res=>{
                setdata(res.data)
          ...
          <tbody>
            ...
            <>
             <tr key={k}>
              ...
              <td>{item.Email}</td>
              <td>{item.Course}</td>
              <td onClick={()=>Deldata(item.id)}><span className="material-symbols-outlined" >delete</span></td>
              <td> <Link to={`/editdata/${item.id}`} ><span className="material-symbols-outlined">edit_square</span></Link></td>
             </tr>
            </>
             ))}
             </tbody>
